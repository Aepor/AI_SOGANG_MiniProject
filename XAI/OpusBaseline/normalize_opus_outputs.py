"""Validate Opus outputs and convert evidence to signed word vectors."""
...
def normalize_record(row: dict[str, Any]) -> dict[str, Any]:
    """Normalize one Opus explanation record."""
    sample_id = str(row.get("sample_id", "")).strip()
    sentiment = str(row.get("sentiment", "")).strip().lower()
    words = row.get("words")
    if not sample_id:
        raise ValueError("Opus row missing sample_id")
    ...
    return {
        ...
        "opus_sentiment": sentiment,
        "opus_vector": l1_normalize(vector),
        "opus_evidence_indices": sorted(evidence_indices),
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    """Parse CLI arguments."""
    parser = argparse.ArgumentParser(description="Normalize Opus explanation JSONL.")
    parser.add_argument("--input", type=Path, default=comparison_dir() / "opus_explanations.jsonl")
    parser.add_argument("--output", type=Path, default=comparison_dir() / "opus_vectors.jsonl")
